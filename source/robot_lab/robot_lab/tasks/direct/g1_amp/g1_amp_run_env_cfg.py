...
class G1AmpRunEnvCfg(G1AmpDanceEnvCfg):
    ...
    # P(low) = 40% standing/start [0,1] (raised from 35% to train stopping)
    command_duration_min: float = 2.0  # seconds (3→2: more cmd transitions for deceleration training)
    command_duration_max: float = 4.0  # seconds (7→4)
    ...
    rew_heading_run: float = -10.0          # face world +X, cos formula, -5→-10 (double pressure to break 7° yaw plateau)
