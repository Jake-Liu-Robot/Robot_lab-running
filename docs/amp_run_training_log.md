...
## 12. 数据集分析与选择（2026-04-15 补充）

### LAFAN1 跑步数据集概览

来源: `lvhaidong/LAFAN1_Retargeting_Dataset` (HuggingFace)，已重定向到 G1 29-DOF。
CSV 格式: 36列 (root_pos3 + root_quat4_xyzw + joints29)，30fps。

| 文件 | 总帧数 | 时长 | 平均速度 | 峰值速度 | 偏航漂移 | 跑步段(>2m/s,>3s) | 站立段数 |
|------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| run1_subject2 | 7135 | 237.8s | 1.21 | 3.53 | +4.1° | 0段 | 15段 |
| run1_subject5 | 7135 | 237.8s | 1.53 | 3.75 | -4.5° | 0段 | 2段 |
| **run2_subject1** | **7345** | **244.8s** | **1.38** | **3.96** | **+7.1°** | **2段** | **21段** |
| run2_subject4 | 7345 | 244.8s | 1.50 | 4.43 | +4.0° | 1段 | 5段 |

### 为什么选择 run2_subject1

- **唯一有 2 段连续跑步 (>2 m/s, >3s) 的数据集**
- 最佳 10s 窗口平均 2.45 m/s（跑步段帧 2059-2278，7.3s，均速 2.56，峰值 3.35）
- 有丰富的站立段（21段），前后都有
- run2_subject4 峰值更高（4.43）但只有 1 段跑步且更短（6.3s）
- run1 系列均速太低，没有持续 >2 m/s 的段

### 选取帧范围 [1943-2564]

选取标准：包含完整的 站立→加速→持续跑步→减速→站立 周期。

| 时间段 | 帧范围 | 速度 | 内容 |
|--------|--------|------|------|
| 0-1s | 1943-1970 | ~0 m/s | 站立 |
| 1-3s | 1970-2030 | 0→2.3 | 加速 |
| 3-14s | 2030-2360 | 2.0-3.35 | 持续跑步（峰值 3.35 m/s） |
| 14-18s | 2360-2480 | 2.5→0.5 | 减速 |
| 18-20.7s | 2480-2564 | ~0 m/s | 站立（质量差，有抖动） |

### 数据集关键问题发现（2026-04-15）

**问题 1：参考轨迹严重弯曲**
```
偏航漂移: -5.2° → -118.3° = 总偏转 113°！
人在跑一个大弧线（几乎 1/3 圈）
20.7s 跑了 ~41m 距离，但净位移只有 ~1m
→ 判别器学到"转弯是正常的" → 奖励方向偏转
→ 这是跑步方向偏转的根本原因（不是 heading 惩罚太弱）
```

**问题 2：站立片段质量差**
```
所有站立段分析:
  关节速度 RMS: 0.88-1.90 rad/s（理想 <0.3）
  倾斜角: 2.5-10°（理想 <2°）
  重心高度: 0.74-0.79m（波动大）
→ 没有一帧是干净的直立站立
→ 判别器无法学到好的站立风格
...
## 13. 数据处理流程（2026-04-16）

### 处理步骤

```
原始 CSV: run2_subject1.csv [1943-2564], 622帧, 30fps
    │
    ▼ csv2npz_run.py (Pinocchio FK, RunPod)
    │
原始 NPZ: g1_run2_subject1_30.npz (622帧)
    │ 问题: 113° 偏航漂移 + 左右不对称
    │
    ▼ straighten_npz.py
    │ 1. 矫直: 每帧去除累积偏航 → 全部面朝 0°
    │ 2. 镜像: 交换左右关节/body → 消除方向偏好
    │
矫直+镜像 NPZ: g1_run2_subject1_30_straight_mirror.npz (1244帧)
    │ ✅ 直线跑步，左右对称
    │
    │ generate_standing.py (Pinocchio FK, RunPod)
    │ → 从 G1 URDF 默认姿态 (joint_pos=0) 计算 FK
    │ → 151帧 (5s)，完美直立站立
    │ → 高度 0.777m，零速度，零关节运动
    │
站立 NPZ: g1_standing_5s.npz (151帧)
    │
    ▼ combine_npz.py (--repeat-standing 3)
    │
最终 NPZ: g1_run_and_stand.npz (1697帧, 56.5s)
    跑步: 1244帧 (73%) — 矫直+镜像
    站立: 453帧 (27%) — FK精确默认姿态 ×3
```
...
```python
# 对每帧:
yaw = extract_yaw(root_quat)        # 提取当前偏航角
inv_yaw = quat_from_yaw(-yaw)       # 反向旋转
root_quat = inv_yaw × root_quat     # 去除偏航 → 面朝 0°
root_vel = rotate(inv_yaw, root_vel) # 速度也旋转
body_pos = rotate(inv_yaw, body_pos) # 位置也旋转
# joint_pos 不变 → 步态完全保留，只去掉路径曲率
```
...
```
交换左右关节索引:
  left_hip(0) ↔ right_hip(6), left_knee(3) ↔ right_knee(9), ...

取反 roll/yaw 关节（镜像翻转方向）:
  hip_roll, hip_yaw, waist_yaw/roll, shoulder_roll/yaw

交换左右 body 索引:
  left_ankle(8) ↔ right_ankle(7), left_knee(13) ↔ right_knee(12), ...

取反 y 分量（镜像平面 XZ）:
  body_pos.y, body_lv.y, quat.y, quat.z, ang_vel.x, ang_vel.z
```

### 合成站立数据详解

```
使用 Pinocchio FK 从 URDF 计算 joint_pos=0 时的精确 body 位置:
  pelvis:       (0, 0, 0.777)   ← 加地面偏移
  left_ankle:   (0, +0.119, 0.020)
  right_ankle:  (0, -0.119, 0.020)
  torso:        (-0.004, 0, 0.821)
  ...

所有速度 = 0 → 完美静止
所有关节 = 0 → URDF 默认直立姿态
→ 判别器学到: "站立 = 直立 + 安静 + 高度 0.78m"
```

---

## 14. 训练 Run 6 — 矫直+站立数据（2026-04-16）

### 14.1 改动总结

| 改动 | 类型 | 内容 |
|------|------|------|
| 矫直轨迹 | 数据 | 去除 113° 偏航漂移 → 直线跑步 |
| 左右镜像 | 数据 | 消除步态方向偏好 → 对称步态 |
| 合成站立 | 数据 | FK 精确默认姿态 → 判别器学好站立 |
| heading | 奖励 | -0.5 → -0.75 (×1.5，原公式不变) |
| 其他奖励 | 奖励 | 全部保持 Run 4b 原值 |

### 14.2 数据修复 vs 奖励调参的教训

```
Run 5 (奖励暴力调参):
  heading ×4, standing ×5, 新增 gait_phase, cmd=0 15%
  → 5K 步崩溃 (episode 43)

Run 5b (温和奖励调参):
  heading ×2, standing ×2, gait_phase, cmd=0 10%
  → 持续下降 (763→593)

Run 6 (修数据):
  矫直+镜像+站立，heading 仅 ×1.5
  → 10K 步 episode=989，快速恢复 ✅
  
教训: 奖励惩罚在对抗判别器时是无效的。
      判别器从弧线跑步数据学到转弯偏好 → heading 惩罚抵消不了
      修数据（矫直）→ 判别器和 heading 协同 → 立即生效
```

### 14.3 训练进度

从 Run 4b 续训的 checkpoint (等效 40K 步)，先用矫直+镜像数据训练 10K 步，再加站立数据继续训练。

```
矫直数据阶段 (10K步):
  5K  → episode=820, disc_loss=2.22
...
Run 4b (40K等效步) — 增强弱奖励
  ...
  基于CSV定量分析的调参
  episode_len 收敛在 ~980
  Eval: 速度跟踪 3.9 m/s ✅, 方向偏转 ⚠️, 站立半蹲 ⚠️

Run 5 (30K步) — 失败（暴力调参）
  heading: -0.5→-2.0(θ²), standing: -0.01→-0.05, standing_height=-10, gait_phase=-1.0
  4项同时改，幅度 4-10x → episode 43 步崩溃
  教训: 多项同时大改 = 必崩

Run 5b (20K步) — 失败（温和但仍太多）
  heading=-1.0(θ²), standing=-0.02, standing_height=-3, gait_phase=-1, cmd=0=10%
